...
Event Flow & \begin{enumerate}
	...
	\item The registration form is loaded and the user compiles it.
	\item The user authorises the personal data treatment.
	\item The user reads the e-mail received by myTaxiService and clicks on the link to confirm the registration.
	\end{enumerate}
\\
\hline
Output condition & The user has been successfully registered, now he or she can login and request a taxi as a passenger or work through myTaxiService as a taxi driver . \\
\hline

Exception &  An exception occurs when one of these is not respected: 

	\begin{itemize}
	\item ~\ref{f-sameInfo}    %The username/e-mail is already use by someone else.
	\item ~\ref{f-usrn}       %The username doesn't respect the restrictions.
	\item ~\ref{f-psw1}     %The password doesn't respect the restrictions.
	\item ~\ref{f-psw2}    %The password entered the second time is different from the first one.
	\end{itemize}
	These exceptions are handled advising the user and reloading the registration form (2 of Event Flow)
	\begin{itemize}
	\item ~\ref{f-dataTreat}   %The user doesn't authorises the data treatment.
	\item ~\ref{f-confirm}   %The user doesn't confirms the registration clicking on the link received via e-mail.
	\end{itemize}
	These are handled deleting all user's registration info: the registration ends failing.
